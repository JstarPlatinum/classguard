...
这个版本已经能够避免固定温度阈值带来的季节问题，同时保留了你提出的“温度区间统计”思路，只是建议将绝对温度分箱改为相对背景温差 `ΔT` 分箱。后续如果采集到足够多的实际场景数据，还可以把 `occupancy_score`、`max_region_area`、`max_delta`、`CO₂ 趋势` 等特征输入轻量分类器，实现更可靠的占用状态判断。
